...
- `GroundedBlock.expected_payoff(policies, measure)`, what a policy profile is
  ...
  what a given one achieved. The measure is required and carries its own
  configuration: `Sampled(n, rng=...)` reduces draws of the block's shocks,
  `Discretized(disc_params)` reduces the nodes of a discretization. The two
  carry different error, so the axis reduced and its size travel back with the
  number as an `ExpectedPayoff`.
