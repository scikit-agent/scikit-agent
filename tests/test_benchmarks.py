...
class TestBenchmarksCatalogue:
    ...
    def test_complete_catalogue_exists(self):
        ...
        expected_models = ["D-1", "D-2", "D-3", "D-4", "U-1", "U-2", "U-3", "U-4", "U-5", "U-6"]
        actual_models = list(BENCHMARK_MODELS.keys())
        
        assert set(expected_models) == set(actual_models), f"Expected {expected_models}, got {actual_models}"
        assert len(BENCHMARK_MODELS) == 10, f"Expected 10 models, got {len(BENCHMARK_MODELS)}"
    ...
    def test_model_registry_structure(self):
        ...
        for model_id in BENCHMARK_MODELS.keys():
            ...
            assert "calibration" in model_info, f"Model {model_id} missing 'calibration'"
            assert "analytical_policy" in model_info, f"Model {model_id} missing 'analytical_policy'"
            
            # Test calibration has description
            assert "description" in model_info["calibration"], f"Model {model_id} missing description"
    
    @pytest.mark.parametrize("model_id", ["D-1", "D-2", "D-3", "D-4", "U-1", "U-2", "U-3", "U-4", "U-5", "U-6"])
    def test_model_validation(self, model_id):
        ...
        if "success" in result:
            assert result["success"] == True, f"Model {model_id} failed validation: {result}"
        else:
            # If there's an error, it should be a meaningful one
            assert "error" in result, f"Model {model_id} returned unexpected result: {result}"
    ...
    def test_deterministic_models(self):
        ...
        assert torch.all(result_d2["c"] <= test_states["W"]), "D-2 consumption should not exceed wealth"
        ...
        assert torch.all(result_d3["c"] < test_states["m"]), "D-3 consumption should be less than cash-on-hand"
        ...
        assert torch.all(result_d4["c"] < test_states["m"]), "D-4 consumption should be less than cash-on-hand"
    ...
    def test_stochastic_models(self):
        ...
        test_states = {"A": torch.tensor([1.0, 2.0, 3.0]), "p": torch.tensor([1.0, 1.0, 1.0])}
    ...
    def test_model_descriptions(self):
        ...
        for model_id, description in models.items():
            assert any(expected in description for expected in expected_descriptions[model_id].split()), \
    ...
    def test_model_access_functions(self):
        ...
        for model_id in BENCHMARK_MODELS.keys():
            ...
            assert isinstance(calibration, dict), f"Calibration for {model_id} is not a dict"
            assert "description" in calibration, f"Calibration for {model_id} missing description"
...
class TestStaticIdentityVerification:
    ...
    def test_u2_cara_affine_rule(self):
        ...
        test_states = {"A": torch.tensor([1.0, 2.0, 3.0]), "y": torch.tensor([1.0, 1.1, 0.9])}
        result = policy(test_states, {}, calibration)
    ...
    def test_u6_riccati_feedback(self):
        ...
        test_states = {"y": torch.tensor([1.0, 1.2, 0.8]), "h": torch.tensor([0.5, 0.6, 0.4])}
        result = policy(test_states, {}, calibration)
...
class TestDynamicOptimalityChecks:
    ...
    def test_hall_euler_equation_simulation(self):
        ...
        for mc_rep in range(5):
            ...
            for t in range(T-1):
                ...
                rhs = quad_a - quad_b * c_path[t+1]  # E[u'(c_{t+1})] = u'(c_{t+1}) for martingale
    ...
    def test_epstein_zin_euler(self):
        ...
        assert torch.all(result["c"] < test_states["m"]), "U-5 consumption should be less than cash-on-hand"
        ...
        for model_id, config in test_cases.items():
            ...
            if model_id in ["D-3", "D-4"]:
                ...
                for t in range(T-1):
                    ...
                    assert torch.allclose(m_path[t+1], expected_m_next, atol=EPS_BUDGET), \
                ...
                for t in range(1, T):
                    ...
                    A_path[t] = A_path[t-1] * R + calibration["y_mean"] - c_path[t-1]
                ...
                for t in range(T):
                    states = {"A": A_path[t:t+1], "y": torch.tensor([calibration["y_bar"]])}
                    result = policy(states, {}, calibration)
                    c_path[t] = result["c"][0]
                    
                    if t < T-1:
                        # Asset evolution: A_{t+1} = (A_t + y_t - c_t)*R
                        A_path[t+1] = (A_path[t] + calibration["y_bar"] - c_path[t]) * R
