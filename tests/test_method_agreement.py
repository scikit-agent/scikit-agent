...
from skagent.ann import BlockPolicyNet, BlockValueNet, train_block_policy_nn, train_bellman_nets
...
class TestMathematicalConsistency:
    ...
    def test_constraint_handling_consistency(self):
        ...
        assert torch.all(violations_good['c_upper'] > 0), "Good consumption should satisfy constraints"
        
        # Bad case should have negative violations (violated)
        assert torch.all(violations_bad['c_upper'] < 0), "Excessive consumption should violate constraints"
    ...
    def test_economic_reasonableness_check(self):
        ...
        edlr_loss = get_expected_discounted_lifetime_reward_loss(
            self.state_variables, self.block, self.discount_factor, big_t, self.test_calibration,
            use_constraints=True  # Enable constraint handling for fair comparison
        )
        edlr_policy = BlockPolicyNet(self.block, width=32)
        edlr_policy = train_block_policy_nn(edlr_policy, edlr_grid, edlr_loss, epochs=200)
        
        # Bellman training with constraints
        bellman_grid = generate_bellman_training_grid(small_config, self.block, n_samples=50)
        bellman_loss = get_bellman_residual_loss(
            self.state_variables, self.block, self.discount_factor, self.test_calibration,
        ...
        bellman_policy, _ = train_bellman_nets(bellman_policy, bellman_value, bellman_grid, bellman_loss, epochs=200)
        
        # Test on reasonable state
        test_states = {'m': torch.tensor([2.0]), 'a': torch.tensor([1.0])}
        test_shocks = self._create_test_shocks(1)
        
        edlr_consumption = edlr_policy.decision_function(test_states, test_shocks, self.test_calibration)['c'][0]
        bellman_consumption = bellman_policy.decision_function(test_states, test_shocks, self.test_calibration)['c'][0]
        
        print(f"Economic reasonableness test:")
        ...
        assert torch.isfinite(bellman_consumption), "Bellman consumption should be finite"
    ...
    def _create_edlr_grid(self, state_config, big_t):
        ...
        for t in range(big_t):
            ...
            for shock_name, shock_values in shocks_t.items():
                shock_dict[f"{shock_name}_{t}"] = torch.tensor(shock_values, dtype=torch.float32, device=device)
        
        combined_labels = list(shock_dict.keys())
        combined_values = torch.stack([shock_dict[label] for label in combined_labels]).T
    ...
    def _create_test_shocks(self, n_points):
        ...
        return {name: torch.tensor(vals, dtype=torch.float32) for name, vals in shocks_raw.items()}
...
class TestTrainingProperties:
    ...
    def test_edlr_training_convergence(self):
        ...
        assert torch.isfinite(torch.tensor(initial_loss)), "Initial EDLR loss should be finite"
        assert torch.isfinite(torch.tensor(final_loss)), "Final EDLR loss should be finite"
        
        # Test that the trained policy produces reasonable consumption decisions
        test_states = {'m': torch.tensor([2.0]), 'a': torch.tensor([1.0])}
        test_shocks = self._create_test_shocks(1)
        consumption = trained_net.decision_function(test_states, test_shocks, self.test_calibration)['c']
        
        assert torch.isfinite(consumption[0]), "EDLR consumption should be finite"
        print(f"EDLR consumption decision: {consumption[0]:.4f} (feasible range: 0 to {test_states['m'][0]:.1f})")
    ...
    def test_bellman_training_convergence(self):
        ...
        bellman_grid = generate_bellman_training_grid(
            self.small_state_config, self.block, n_samples=20, parameters=self.test_calibration
        )
        ...
        initial_loss = self._evaluate_bellman_loss(policy_net, value_net, bellman_grid, bellman_loss)
        
        trained_policy, trained_value = train_bellman_nets(
            policy_net, value_net, bellman_grid, bellman_loss, epochs=100
        )
        
        final_loss = self._evaluate_bellman_loss(trained_policy, trained_value, bellman_grid, bellman_loss)
        
        # Training should reduce loss
        assert final_loss < initial_loss, f"Bellman training should reduce loss: {initial_loss:.4f} -> {final_loss:.4f}"
    ...
    def test_economic_reasonableness(self):
        ...
        edlr_consumption = edlr_policy.decision_function(test_states, test_shocks, self.test_calibration)['c']
        bellman_consumption = bellman_policy.decision_function(test_states, test_shocks, self.test_calibration)['c']
        ...
        print(f"EDLR consumption: {edlr_consumption[0]:.4f} (max feasible: {max_consumption:.4f})")
        ...
        assert torch.isfinite(bellman_consumption[0]), "Bellman consumption should be finite"
    ...
    def _train_bellman_policy(self):
        ...
        bellman_grid = generate_bellman_training_grid(self.small_state_config, self.block, n_samples=20, parameters=self.test_calibration)
        bellman_loss = get_bellman_residual_loss(
            self.state_variables, self.block, self.discount_factor, self.test_calibration
        )
        policy_net = BlockPolicyNet(self.block, width=16)
        value_net = BlockValueNet(self.block, self.state_variables, width=16)
        return train_bellman_nets(policy_net, value_net, bellman_grid, bellman_loss, epochs=50)
    ...
    def _evaluate_bellman_loss(self, policy_net, value_net, grid, loss_fn):
        ...
        loss = loss_fn(policy_net.get_decision_function(), value_net.get_value_function(), grid)
        return loss.mean().item()
...
class TestConstraintEffectiveness:
    ...
    def test_fischer_burmeister_constraint_handling(self):
        ...
        bellman_loss_std = get_bellman_residual_loss(
            self.state_variables, self.block, self.discount_factor, 
            self.test_calibration, use_fischer_burmeister=False
        )
        
        # Train with Fischer-Burmeister constraints
        bellman_loss_fb = get_bellman_residual_loss(
            self.state_variables, self.block, self.discount_factor,
            self.test_calibration, use_fischer_burmeister=True
        )
        
        # Quick training comparison
        grid = generate_bellman_training_grid(small_config, self.block, n_samples=10, parameters=self.test_calibration)
        ...
        loss_std = bellman_loss_std(policy_net.get_decision_function(), value_net.get_value_function(), grid)
        loss_fb = bellman_loss_fb(policy_net.get_decision_function(), value_net.get_value_function(), grid)
        
        assert torch.all(torch.isfinite(loss_std)), "Standard loss should be finite"
        assert torch.all(torch.isfinite(loss_fb)), "Fischer-Burmeister loss should be finite"
...
class TestMethodAgreement:
    ...
    def test_method_agreement_with_extensive_training(self):
        ...
        test_grid = Grid({'m': {'min': 2.0, 'max': 2.0, 'count': 1}, 'a': {'min': 1.0, 'max': 1.0, 'count': 1}})
        test_states = test_grid.to_dict()
        test_shocks = self._create_test_shocks(1)
        
        edlr_c = edlr_policy.decision_function(test_states, test_shocks, self.test_calibration)['c'][0]
        bellman_c = bellman_policy.decision_function(test_states, test_shocks, self.test_calibration)['c'][0]
        
        print(f"Extensive training comparison:")
        ...
        if disagreement > 1.0:
            ...
            print("This indicates potential training or implementation issues to investigate")
    ...
    def _train_bellman_extensively(self, state_config):
        ...
        bellman_grid = generate_bellman_training_grid(state_config, self.block, n_samples=50, parameters=self.test_calibration)
        bellman_loss = get_bellman_residual_loss(
            self.state_variables, self.block, self.discount_factor, self.test_calibration,
        ...
        return train_bellman_nets(policy_net, value_net, bellman_grid, bellman_loss, epochs=200)
