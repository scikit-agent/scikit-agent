...
class TestTheBridgeIsChecked:
    ...
    def test_a_bridge_that_reduces_over_its_class_raises(self):
        with pytest.raises(ValueError, match="arrival state that value becomes"):
    ...
    def test_an_array_reaching_an_axis_free_arrival_state_raises(self):
        """INFLATE, the mirror of the collapse and the loud one: a bridge that
        forgets to reduce. Recorded because the enumeration should say which
        errors need a check of their own and which already announce themselves."""
        household = DBlock(
            name="hh",
            dynamics={
                "z": lambda W, a: W + a,
                "a": lambda z: z * 0.7,
                "savings": lambda z: z * 0.3,
            },
        )
        block = RBlock(
            name="inflate",
            blocks=[
                DBlock(name="market", dynamics={"W": lambda kl: 1.0 + kl}),
                RBlock(name="hs", entity=Entity("household"), blocks=[household]),
                DBlock(name="tick", dynamics={"kl": lambda savings: savings}),
            ],
        )
        sim = Simulator(
            {"household": 8},
            block,
            {},
            {"a": Uniform(low=1.0, high=5.0), "kl": Uniform(low=1.0, high=2.0)},
            sample_count=2,
            T_sim=2,
            seed=0,
        )
    ...
    def test_a_value_indexed_by_the_wrong_class_raises(self):
        ...
        population handed a value indexed by another."""
        block = RBlock(
            name="cross",
            blocks=[
        ...
        with pytest.raises(ValueError, match=r"expected \(2, 8\)"):
            sim.simulate()
