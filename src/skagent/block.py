...
class RBlock(Block):
    ...
    def __post_init__(self):
        """Reject generated randomizers that collide across sub-blocks."""
        leaves = list(self.iter_dblocks())

        declared = set()
        for block in leaves:
            declared.update(block.shocks)
            declared.update(block.dynamics)
            declared.update(block.reward)
        for block in leaves:
            for sym, rule in block.dynamics.items():
                randomizer = _randomizer_name(sym)
                if (
                    isinstance(rule, Control)
                    and rule.randomizes
                    and randomizer in declared
                ):
                    raise ValueError(
                        f"randomizing control {sym!r} generates shock "
                        f"{randomizer!r}, but that name is already declared"
                    )
