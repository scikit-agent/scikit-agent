...
dynamics = {
    ...
    "p": lambda p_prev, psi, PermGroFac: p_prev
    * psi
    * PermGroFac,  # Permanent income evolution
    # === Portfolio Returns ===
    "R_portfolio": lambda alpha, Rfree, risky_return: (  # Portfolio return
        Rfree + alpha * (risky_return - Rfree)
    ),
    # === Budget Constraint ===
    "b": lambda a_prev, R_portfolio: a_prev * R_portfolio,  # Beginning-of-period wealth
...
consumption_portfolio_block = ska.DBlock(
    ...
    shocks=shocks,
    dynamics=dynamics,
    reward={"u": "household"},  # Household maximizes utility
)
...
def consumption_rule(m, p):
    ...
    mpc = 0.1 + 0.05 * min(m / p, 5.0)  # MPC between 10-35%
    return mpc * m
...
def portfolio_rule(a, p):
    ...
    if a <= 0:
        return 0.0
    # Portfolio share in risky asset depends on wealth-to-income ratio
    wealth_ratio = a / p
    risky_share = min(0.8, 0.3 + 0.1 * wealth_ratio)  # 30-80% in stocks
    return risky_share
...
# Initial conditions
initial_conditions = {
    "a_prev": calibration["init_wealth"],  # Start with some wealth
    "p_prev": calibration["init_perm"],  # Initial permanent income
}
...
if assets is not None:
    ...
    axes[1, 1].hist(final_wealth, bins=50, alpha=0.7, color="gold", edgecolor="black")
    axes[1, 1].axvline(
